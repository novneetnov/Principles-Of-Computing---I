...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    while current_day <= days_in_simulation:
        counter += 1
        days_to_next_bribe = math.ceil(float(current_bribe_cost - savings) / current_salary)
        if days_to_next_bribe < 0:
            days_to_next_bribe = 0

        current_day += days_to_next_bribe
        earnings += (days_to_next_bribe * current_salary)
        earnings /= math.exp(0.095 * current_day)
        savings += days_to_next_bribe * current_salary
        savings -= current_bribe_cost
